...
def test_lookup_at_2024_lower_boundary_uses_prior_business_day_in_seed_year() -> None:
    symbol = "LME_ALU_CASH_SETTLEMENT_DAILY"
    _insert_price(symbol=symbol, settlement_date=date(2023, 12, 29), price_usd=222.0)

    with SessionLocal() as session:
        quote = get_cash_settlement_price_d1_with_provenance(
            session, symbol=symbol, as_of_date=date(2024, 1, 2)
        )
        assert quote.settlement_date == date(2023, 12, 29)
        assert quote.value == Decimal("222.0")
...
def test_market_calendar_is_in_repo_year_keyed_not_holidays_dependency() -> None:
    ...
    assert 2023 in _LME_HOLIDAYS_BY_YEAR
