...
- On refusal: emit the matching HMAC-signed audit event **before** raising, with the **event-specific** payload defined in governance (NOT a single unified payload) — `order_rejected_kind_mismatch` → `{commercial_partner_id, kind, order_type, requesting_actor_sub}`; `order_rejected_kyc_not_approved` → `{commercial_partner_id, kind, order_type, kyc_status_observed, requesting_actor_sub}`; `order_rejected_sanctions_blocked` → `{commercial_partner_id, kind, order_type, sanctions_status_observed, requesting_actor_sub}`. Then raise **HTTP 422**. (All three refusal classes MUST produce an audit event — the kind-mismatch case is not exempt.)
...
| sanctions_status | Enum(`unscreened`,`clear`,`flagged`,`blocked`) | default `unscreened` (NOT `clear`); screening writes `clear`/`flagged`/`blocked`. A partner cannot reach `kyc_status=approved` without an effective `clear` (a `clear` screening OR a risk_manager adjudication of a `flagged` result; §6), so an `unscreened` or unresolved-`flagged` partner cannot be APPROVED. (The order gate itself is `sanctions_status != blocked` per D3, so a partner already `approved` then re-screened to `flagged` still passes the order gate — `flagged` blocks new KYC approval, not order creation for an already-approved partner.) |
...
**`sanctions_adjudications`** (append-only, immutable): `{id, partner_type, partner_id, superseded_screening_id, decision (`clear`|`blocked`), reason, adjudicating_actor_sub, adjudicated_at}` — a risk_manager override of a `flagged` screening; sets `sanctions_status` without mutating the screening row. Current `sanctions_status` = latest of {successful screening result, adjudication} by timestamp. Adjudication is VALID ONLY against the LATEST screening while it is still `flagged` — a `flagged` already superseded by a newer `blocked`/`clear` screening cannot be adjudicated (prevents a stale flagged from overriding a newer hard hit).

### 5.3 Migration (numeric chain, single-head; ENUM lifecycle care)
- Create enums + `commercial_partners` + `sanctions_screenings` + `sanctions_adjudications`. Named ENUMs explicitly created before `ALTER TABLE ADD COLUMN`; explicit `CAST(... AS <enum>)` for text literals (per CLAUDE.md fresh-Postgres rules).
...
- **Pre-move validation (fail-closed, hedge-side refs)**: symmetrically, enumerate hedge-domain references — `RFQInvitation.counterparty_id`, `RFQQuote.counterparty_id`, `HedgeContract.counterparty_id`, `llm_decision_artifact.counterparty_id` — that point at a customer/supplier `counterparties` row being migrated out (a data-quality artifact, since the RFQ create path historically loaded any `Counterparty` without a type guard). If any exist, the migration HALTS with a remediation report and the customer/supplier rows are NOT removed until those references are corrected. NO silent FK breakage.
...
- **`sanctions_screening_service`** — `POST https://api.opensanctions.org/match/sanctions?algorithm=logic-v2`, header `Authorization: ApiKey <OPENSANCTIONS_API_KEY>`, request body = OpenSanctions `EntityMatchQuery` envelope `{"queries": {"q1": {"schema": "Company", "properties": {"name": [..], "jurisdiction": [<country>], "registrationNumber": [<tax_id>], "leiCode": [<lei>]}}}}` (top-level `queries` map, ARRAY-valued properties — a bare entity body 422s). Map response `match`/`score` → `result` (consistent with the governance ranges): no match OR match below the REVIEW threshold → `clear` (low-confidence noise, NOT flagged); match ≥ review but < HARD threshold → `flagged` (risk_manager adjudicates); match ≥ HARD threshold → `blocked`. Thresholds defined in the Wave-2 dispatch. Persist immutable `sanctions_screenings` row + update partner `sanctions_status`. **Hard-fail** (record `status=error` on a SEPARATE committed session — dual-session commit-before-raise, since `unit_of_work` rolls back the request session on exception — then raise) on API/network error — never set `clear` silently. `OPENSANCTIONS_API_KEY` required in prod/staging via an APP_ENV-gated boot validator (same shape as `AUDIT_SIGNING_KEY`). Decoupled triggers: on create, manual `POST /commercial-partners/{id}/screen` and `POST /counterparties/{id}/screen` (hedge, risk_manager), and a scheduled daily re-screen in the existing `scheduler` service (covers both domains), attributed to the new `service:sanctions_screening` identity (added to the AUTHORIZATION MATRIX in W0).
...
- Full KYC documentary suite (`KycDocument`, `CreditCheck`, `KycCheck` attestation models) — remains the separate post-pilot concern noted in the governance.md **"Schema (binding)"** note (within the commercial KYC + order gate section).
