...
- On refusal: emit the matching HMAC-signed audit event **before** raising — `order_rejected_kind_mismatch` (kind coherence fails), `order_rejected_kyc_not_approved` (kyc leg), or `order_rejected_sanctions_blocked` (sanctions leg) — with payload `{commercial_partner_id, kind, order_type, kyc_status_observed, sanctions_status_observed, requesting_actor_sub}`. Then raise **HTTP 422**. (All three refusal classes MUST produce an audit event — the kind-mismatch case is not exempt.)
