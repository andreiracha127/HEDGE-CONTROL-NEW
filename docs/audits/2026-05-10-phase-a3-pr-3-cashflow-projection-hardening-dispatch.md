...
- [ ] **Pre-fix test cleanup completed** (per §7.1):
  ...
  - `grep -n "no_settlement_date_uses_as_of" backend/tests/test_cashflow_projection_service.py` returns ZERO matches (the test pinning the removed `settlement_date or as_of_date` substitution is DELETED — replacement is `test_projection_raises_422_when_contract_settlement_date_missing` in §7.2).
...
3. Update every `@patch(MARKET_PRICE_PATCH, return_value=None)` call site: replace with `@patch(MARKET_PRICE_PATCH, side_effect=PriceReferenceUnprovable("..."))` — the new function raises, it does not return None. **Includes `test_empty_projection_returns_zero_summary`** even though that test never triggers the mock (empty DB → no per-row lookups → `_get_market_price_quote` never called). Convert anyway to satisfy the §6 acceptance grep `never return_value=None` — the `side_effect` will not fire and the test remains valid.
...
6. **DELETE `test_contract_no_settlement_date_uses_as_of`** (verified via `read_file` at line ~346 of the existing test file). This test fixture creates a contract with `settlement_date=None`, calls `compute_cashflow_projection`, and asserts `len(result.items) == 1` — pinning the exact `contract.settlement_date or as_of_date` substitution that §3.2 removes and §10 prohibits. After §3.2, the same fixture raises `HTTPException(422)`; the test would hard-fail post-fix. Replacement: `test_projection_raises_422_when_contract_settlement_date_missing` already prescribed in §7.2. Grep verification: `grep -n "no_settlement_date_uses_as_of" backend/tests/test_cashflow_projection_service.py` MUST return ZERO matches post-cleanup.
