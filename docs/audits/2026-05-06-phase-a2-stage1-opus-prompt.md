...
> **Instruções operacionais.** Você roda em Claude Code CLI no diretório do projeto `d:\Projetos\Hedge-Control-New`. Tem acesso a Read/Grep/Glob/Serena. Você está sendo despachado em paralelo com o Auditor B (Gemini 3.1 Pro), com o **mesmo prompt**. Você NÃO vê o output do Auditor B; ele NÃO vê o seu. A independência é institucional. Um Jury (GPT 5.5) recebe os 2 outputs em Stage 3 e adjudica.
...
(Use `Write` tool. Sobrescreve se existir. Não crie outros arquivos.)
...
### X-A2-OPUS-01 — <Boundary condition>
...
- Tools used: Read N times, Grep N times, Serena N times.
...
Salve o arquivo em `docs/audits/2026-05-06-phase-a2-findings-opus.md`. Reporte ao orquestrador o caminho do arquivo + commit SHA + summary de tiers (Tier 1: N, Tier 2: N, ...). **PARE.**

- NÃO leia o output do Auditor B (não existirá ainda; e mesmo se existisse, está fora de escopo).
- NÃO comente sobre a metodologia 3-stage.
- NÃO proponha próximos passos.
- NÃO commit, NÃO push, NÃO branch.

O orquestrador agrega seu output + Auditor B + entrega ao Jury (GPT 5.5) em Stage 3.
