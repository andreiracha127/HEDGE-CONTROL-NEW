...
**Layer-boundary contract (Codex catch on PR #74 v4)**: verified at HEAD `ea08d9868` that `compute_commercial_snapshot` at `exposure_service.py:87` currently returns `list[dict]` (raw dicts keyed by string field names like `"pre_reduction_commercial_active_mt"`). The new primitive `compute_commercial_exposure_pure` per §4.1 returns `list[CommercialExposureRead]` (Pydantic DTO). The refactored `compute_commercial_snapshot` must **NOT** silently change its return type — that would break every caller that does dict-key access (route serializers via `response_model` are typically tolerant of either, but `rfq_service.py` and other internal callers may do `row["commodity"]` style access).

The mandatory shape: **`compute_commercial_snapshot` keeps returning `list[dict]`** (signature unchanged). After delegating aggregation to the primitive, it converts each `CommercialExposureRead` back to a dict via `.model_dump()` before returning, OR the primitive offers a `_pure_to_dict(...)` adapter. Same rule for `compute_global_snapshot`. The `list[dict]` ↔ `list[Pydantic model]` boundary is owned by the snapshot methods, not the primitive. Verify every caller of `compute_commercial_snapshot` and `compute_global_snapshot` (sweep `rg -nP "compute_commercial_snapshot|compute_global_snapshot" backend/app`) and assert no caller is updated as part of this wave — they all see the same return shape they see today.

Behavior must be byte-equivalent to today on every existing fixture. The existing tests in `backend/tests/test_exposures_commercial.py` (covers `compute_commercial_snapshot`) and `backend/tests/test_exposures_global.py` (covers `compute_global_snapshot`) should not need changes; if they do, the refactor introduced a regression (either in aggregation rules OR in the dict↔DTO conversion). `backend/tests/test_exposure_engine.py` and `backend/tests/test_compute_net_exposure.py` cover the engine and net-exposure paths respectively and are not directly exercised by this refactor, but must continue to pass.

### 4.3 Refactor scenario input boundary in `backend/app/services/scenario_whatif_service.py`

#### 4.3.1 Lifecycle filters at `_load_*`

**Critical: there are two separate unfiltered query paths to address (Codex catch on PR #74 v4)**. `run_what_if` at `scenario_whatif_service.py:449-452` performs **inline** `db.query(Order)...all()` and `db.query(HedgeContract)...all()` calls that **do not** go through `_load_orders` / `_load_contracts`. The standalone `_load_contracts` helper at line 224 is currently **dead code** — never called from `run_what_if`. The standalone `_load_orders` at line 212-220 IS called at line 459 (with `order_overrides`) to build the post-delta tuples list, so it has a real downstream consumer. Therefore both the inline queries AND the helper must be addressed:

- **`_load_orders`** at `:212-220`: change `db.query(Order).order_by(Order.created_at.asc()).all()` to `db.query(Order).filter(Order.deleted_at.is_(None)).order_by(Order.created_at.asc()).all()`. The archived-orders block becomes invisible at the post-delta tuple build.
- **`run_what_if:449` `base_orders` inline query**: also filter `Order.deleted_at.is_(None)` (or refactor to call a new `_load_base_orders(db) -> list[Order]` helper that wraps the same filtered query — the `base_orders` consumer at `_apply_deltas` needs raw `Order` rows, not the `(Order, Decimal)` tuples that `_load_orders` returns, so a separate helper is the cleanest shape).
- **`base_contracts` inline query at `run_what_if:450-452` + dead-code `_load_contracts` at `:224`**: **DO NOT** unconditionally filter `base_contracts` to `{active, partially_settled}` — verified at HEAD `ea08d9868` that `base_contracts` is consumed by both `_apply_deltas` (line 455-457) AND assigned to `contracts` (line 460) which feeds the scenario P&L loop at `:541-592`, where settled / cancelled contracts contribute **zero unrealized MTM** (line 542-543) but **still emit realized P&L** from `CashFlowLedgerEntry` rows (lines 561-581). Filtering would silently drop settled hedges from `pl_snapshots` — outside this wave's "exposure aggregation only" scope (Codex P2 catch on PR #74 v3, see §12 calibration).

  The standalone `_load_contracts` at line 224 is **dead code** — `run_what_if` does not call it. The fix shape:

  - **Replace the inline `base_contracts` query** at lines 450-452 with a call to `_load_contracts(db)` so the helper is no longer dead. `_load_contracts` itself stays unfiltered (`session.query(HedgeContract).order_by(HedgeContract.created_at.asc()).all()`). `base_contracts` continues to feed `_apply_deltas` and the P&L loop unchanged.
  - **Add a new helper `_load_exposure_contracts(db: Session) -> list[HedgeContract]`** that applies the full exposure-aggregation filter: `session.query(HedgeContract).filter(HedgeContract.deleted_at.is_(None), HedgeContract.status.in_([HedgeContractStatus.active, HedgeContractStatus.partially_settled])).all()`. Status filter mirrors `exposure_service.py:351-378`. The exposure aggregation path (call to `compute_global_exposure_pure`) consumes this filtered helper.
  - **`_apply_deltas` (`scenario_whatif_service.py:155`, body covering lines 155-211)** continues to receive `base_contracts` (from `_load_contracts`, unfiltered). `_apply_deltas` builds the collision-check set for `AddUnlinkedHedgeContractDelta` — it must see the **full** contract universe (including archived and settled) so a virtual delta with an id matching an existing-but-inactive contract still triggers the 409 collision guard. Piping `_load_exposure_contracts` here would silently bypass the 409.
  - **The MTM/P&L loop at `:541-592`** continues to consume `contracts = base_contracts` (unfiltered). Settled hedges keep emitting their realized cashflow-ledger contributions.

  This preserves J-CL1-04's "scenario exposure baseline matches live A1" guarantee for the exposure surface while leaving scenario P&L + delta-collision semantics untouched. Three call sites of contract loading exist post-fix: `_load_contracts(db)` (raw, used by `_apply_deltas` + P&L loop) called once in `run_what_if`, `_load_exposure_contracts(db)` (filtered, used by exposure aggregation) called once. Runtime cost: one extra `SELECT` per scenario request.
...
7. **`test_settled_hedge_preserved_in_scenario_pl_excluded_from_exposure`**: same fixture as test 4 — a settled HedgeContract with realized P&L from `CashFlowLedgerEntry` rows over the scenario period. Call scenario, assert **both halves of the wiring**:
   - **P&L preservation**: the settled hedge **DOES** appear in `pl_snapshots` with `unrealized_mtm == 0` and `realized_pl == sum(ledger_entries)`.
   - **Exposure exclusion**: the settled hedge **does NOT** appear in `global_exposure_snapshot` (no row with this hedge's commodity contributed via this contract). Compare against a fixture where a second `active` hedge of the same commodity exists — only the active one shows in the exposure snapshot, both show in `pl_snapshots`.

   The two-half assertion is the regression guard for the **wiring** of the two-loader split (Codex catch on PR #74 v4: a buggy implementation where the exposure path wires through the unfiltered `_load_contracts` would let test 7 trivially pass on the P&L assertion alone). Asserting both presence in P&L AND absence in exposure proves the executor wired `_load_exposure_contracts` to the exposure aggregation and `_load_contracts` to the P&L loop — not the inverse, not both to the same loader.

   **This test is mandatory; counting parametrized variants of test 6 as one test, the total file has 7 distinct test functions.**
