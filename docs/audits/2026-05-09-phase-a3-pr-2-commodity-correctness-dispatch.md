...
- [ ] OpenAPI + `schema.d.ts` regenerated to reflect the new required `commodity` field on `AddUnlinkedHedgeContractDelta`. The `MTMResultResponse.price_quote` schema is unchanged (`PriceQuote | None`, `schemas/mtm.py:25`); the scenario-only "always populated" invariant is a runtime guarantee enforced by the §3.7 plumbing + §7 tests, NOT an OpenAPI-level non-nullability claim — making `price_quote` globally required would break non-scenario MTM call sites that legitimately leave it unset.
...
- `backend/tests/test_scenario_whatif_run.py` (extension):
  - `test_scenario_add_unlinked_hedge_contract_requires_commodity` — POST `/scenario/what-if/run` with delta missing `commodity` → 422 with field-level error.
...
11. **Provenance plumbing (§3.7)**:
    ...
    - Update imports per §3.7.5 — add `PriceQuote`, `PriceReferenceUnprovable`, and `get_cash_settlement_price_d1_with_provenance` to the existing `from app.services.price_lookup_service import (...)` block (the re-export path; both `PriceQuote` and `PriceReferenceUnprovable` are re-exported from `price_lookup_service.py:15`). DO NOT add a separate `from app.utils.price_reference import ...` line — the codebase routes through `services.price_lookup_service` per the layering convention. Remove import of `get_cash_settlement_price_d1` (verify no remaining references via grep).
