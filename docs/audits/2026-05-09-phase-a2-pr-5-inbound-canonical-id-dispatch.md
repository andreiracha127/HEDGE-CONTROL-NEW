...
_CANONICAL_ID_RE = re.compile(r"RFQ#(?P<num>RFQ-\d{4}-\d{6})(?!\w)")
...
- The trailing **`(?!\w)` negative lookahead is mandatory** — without it, `re.search` would match malformed inputs as their 6-digit prefixes and route the message to a real older RFQ. `\w` is `[A-Za-z0-9_]` in Python's default flags, so the lookahead rejects every adjacent word character: digit (post-overflow `RFQ#RFQ-2026-1234567` truncating to `RFQ-2026-123456`; adversarial digit-prepend `RFQ#RFQ-2026-0001234` truncating to `RFQ-2026-000123`), letter (`RFQ#RFQ-2026-000123A`), and underscore (`RFQ#RFQ-2026-000123_`). The lookahead enforces "exactly 6 digits, followed by a non-word character (whitespace, punctuation) or end-of-string". A weaker `(?!\d)` would still admit alphanumeric suffixes; a `\b` would similarly admit `_` because underscore is a word character. All three malformed shapes must land in the `no_canonical_id` path — the §6 hard-fail-rather-than-fallback rule at the parser boundary.
...
- `test_parse_canonical_id_rejects_alphanumeric_suffix` — `"RFQ#RFQ-2026-000123A"` and `"RFQ#RFQ-2026-000123_"` and `"RFQ#RFQ-2026-000123abc"` all → `None`. A weaker `(?!\d)` lookahead would let these through (only digits blocked); the `(?!\w)` lookahead rejects letters and underscores too. Test pins the full word-character boundary, covering the canonical-id-is-exactly-RFQ#RFQ-YYYY-NNNNNN contract from §10.
