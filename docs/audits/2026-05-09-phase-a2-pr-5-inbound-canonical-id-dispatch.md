...
**Codex P2 absorbed against commit `69e05ea51` — multi-id status enumeration sweep.** The round-8 introduction of `multi_canonical_id` updated the §3.3 status table and §3.2 step 1 logic but missed five downstream enumerations: §3.3 prose ("three new status values" → four; "these three statuses" routed to operator queue → five), §4 Scope OUT ("four parked statuses" → five with `multi_canonical_id`), and §5 Constitutional rules (the parked-status enumeration). An implementer or downstream A4 consumer reading those would not include `multi_canonical_id` in queue routing, dropping the forwarded-thread safety case at the queue layer instead of the parser layer — re-creating J-A2-06 mixed-RFQ hazard one boundary downstream. **Fix:** every enumeration of parked statuses across §3.3, §4, §5 now includes `multi_canonical_id` explicitly. Self-blame: this is the FOURTH cross-section sweep miss in the cycle (rounds 4, 5, 8, 9). The disciplinary pattern is now unmistakable — when adding a status enum value, every prose paragraph that ENUMERATES status values must be re-grepped, not just the prescriptive table.

**Two Codex P2 absorbed against commit `3b672a79a` — sweep stale §10/§7/§4 refs after multi-id rename.** The round-8 helper rename `_parse_canonical_id` → `_parse_canonical_ids` updated §3.1, §3.2, §3.3, §6, §11 but missed three downstream surfaces: (1) §10 still said "DO NOT introduce `re.findall`-based multi-id handling — extras ignored", which directly contradicts the new §3.2 step 1 multi-id park flow and would let an implementer reintroduce first-wins correlation; (2) §7's 11 parser tests were still named `test_parse_canonical_id_*` and asserted `→ None` for misses, which would fail against the list-returning helper or pressure the implementation back to the old `Optional[str]` contract; (3) §4 Scope OUT said "multi-id handling out of scope", contradicting that PR-5 DOES detect and park multi-id. **Fix:** §10 multi-id constraint rewritten to forbid first-wins correlation while permitting `re.finditer` scanning; §7 tests pluralized (`_parse_canonical_ids_*`) with list-return expectations (`[]` for misses, `["..."]` for matches); §4 Scope OUT clarified to forbid auto-correlating in multi-id case while acknowledging the parker scans all matches; two new §7 tests added for multi-id detection (`returns_all_distinct_matches_for_multi_id`) and quoted-self-outbound (`returns_repeated_for_quoted_self_outbound`). Self-blame: this is the third §10/§7 sweep miss in the cycle (rounds 4, 5, 8 had analogous failures); cross-section sweep discipline must include both prescriptive section + constraint section + test section in every helper-rename or status-taxonomy change.
...
PR-5 introduces four new status values returned by `_process_single_message`:
...
The downstream `webhook_processor` (Phase A4 X-A2-J-01 surface) is the consumer that routes all four new parked statuses (`no_canonical_id`, `multi_canonical_id`, `canonical_id_unknown`, `phone_mismatch`) plus the pre-existing `rfq_archived` to the operator queue. PR-5's responsibility ends at returning the structured status; **PR-5 does NOT modify webhook_processor**, does NOT add a parked-message persistence table, does NOT add a UI surface for the parked queue. Those are A4. The forwarded-thread case in particular MUST land in the operator queue — silently dropping `multi_canonical_id` would re-create the J-A2-06 mixed-RFQ hazard at the queue layer instead of the parser layer.
...
- **Raw inbound message durability (X-A2-J-01)** — Phase A4. PR-5 does NOT add a parked-message persistence table, does NOT modify `webhook_processor.py`, does NOT change the inbound queue surface. The five parked statuses (`no_canonical_id`, `multi_canonical_id`, `canonical_id_unknown`, `phone_mismatch`, plus the pre-existing `rfq_archived`) are returned to whatever caller consumes `_process_single_message`'s dict; the persistence layer is owned by A4.
...
- **Auto-correlating multi-canonical-id messages to one of the matched RFQs** — out of scope. PR-5 detects multi-id (forwarded threads) and parks via `multi_canonical_id`; auto-resolving which RFQ to correlate to in the multi-id case is a future feature requiring operator policy decisions, not PR-5's scope. (Note: the parser DOES scan all matches via `re.finditer` to enable the park decision; the §10 constraint is against silently *correlating* with extras ignored, not against scanning.)
...
- **§2.7 — Output Contract** (governance.md:208-217) — "Audit-friendly. Free of speculation." Each parked status (`no_canonical_id`, `multi_canonical_id`, `canonical_id_unknown`, `phone_mismatch`) carries enough structured fields (canonical_number / canonical_numbers, rfq_id, from_phone, message_id) for the operator queue to disambiguate the failure mode at audit time. `multi_canonical_id` returns `canonical_numbers` (sorted list) so the operator can see exactly which RFQs were mixed.
...
- `test_parse_canonical_ids_extracts_from_prefixed_body` — `"RFQ#RFQ-2026-000123 — your quote please"` → `["RFQ-2026-000123"]`.
- `test_parse_canonical_ids_handles_whitespace_prefix` — `"  RFQ#RFQ-2026-000123 ..."` → `["RFQ-2026-000123"]` (the outbound helper at `rfq_service.py:83-96` strips leading whitespace via `lstrip()` before the canonical check; inbound parser must tolerate the same with `re.finditer`).
- `test_parse_canonical_ids_handles_internal_position` — `"Olá! RFQ#RFQ-2026-000456 está confirmado"` → `["RFQ-2026-000456"]` (regex matches mid-body, supporting quote-replies).
- `test_parse_canonical_ids_returns_empty_list_on_missing` — `"Bom dia, segue minha cotação"` → `[]`. No exception; no log; no DB.
- `test_parse_canonical_ids_returns_empty_list_on_empty_or_none` — `""` and `None` both return `[]` without exception.
- `test_parse_canonical_ids_rejects_bare_rfq_number_without_hash` — `"RFQ-2026-000123"` (no `#`) → `[]`.
- `test_parse_canonical_ids_rejects_short_sequence` — `"RFQ#RFQ-2026-12345"` (5 digits) → `[]`.
- `test_parse_canonical_ids_rejects_overlong_sequence_post_overflow` — `"RFQ#RFQ-2026-1234567"` (7 digits, simulating post-`:06d`-overflow when sequence ≥ 1_000_000) → `[]`. Without the `(?!\w)` lookahead (or `(?!\d)`, as digits are a subset of word chars) the parser would silently truncate to `RFQ-2026-123456` and route a future canonical id to an unrelated older RFQ; this test pins the hard-fail.
- `test_parse_canonical_ids_rejects_adversarial_digit_prepend` — `"RFQ#RFQ-2026-0001234"` (extra leading digit, 7 total) → `[]`. Same lookahead concern: without `(?!\w)` the parser would extract `RFQ-2026-000123` (the 6-digit prefix), routing the malformed input to a real older RFQ. Test pins the boundary.
- `test_parse_canonical_ids_rejects_alphanumeric_suffix` — `"RFQ#RFQ-2026-000123A"` and `"RFQ#RFQ-2026-000123_"` and `"RFQ#RFQ-2026-000123abc"` all → `[]`. A weaker `(?!\d)` lookahead would let these through (only digits blocked); the `(?!\w)` lookahead rejects letters and underscores too. Test pins the full trailing word-character boundary, covering the canonical-id-is-exactly-RFQ#RFQ-YYYY-NNNNNN contract from §10.
- `test_parse_canonical_ids_rejects_adjacent_word_prefix` — `"abcRFQ#RFQ-2026-000123"` and `"_RFQ#RFQ-2026-000123"` and `"123RFQ#RFQ-2026-000456"` all → `[]`. Without the leading `(?<!\w)` lookbehind, `re.finditer` would find the canonical-id substring inside an adjacent word-char prefix and route the message; the symmetric leading boundary makes the canonical id a STANDALONE token (or internal to non-word characters) only.
- `test_parse_canonical_ids_returns_all_distinct_matches_for_multi_id` — `"RFQ#RFQ-2026-000123 — 2550\nRFQ#RFQ-2026-000456 — 2600"` → `["RFQ-2026-000123", "RFQ-2026-000456"]` (order-preserving). Pins the multi-id detection that §3.2 step 1 routes to `multi_canonical_id` park.
- `test_parse_canonical_ids_returns_repeated_for_quoted_self_outbound` — `"RFQ#RFQ-2026-000123 — RFQ#RFQ-2026-000123 — ok"` → `["RFQ-2026-000123", "RFQ-2026-000123"]` (length 2, but `set()` of length 1; §3.2 step 1 dedupes to single-RFQ correlation, NOT a forwarded thread).
...
- **DO NOT** correlate first-id-wins when multiple distinct canonical ids are present. The parser scans all matches via `re.finditer`; §3.2 step 1 dedupes via `set(...)` and routes 2+ distinct ids to `multi_canonical_id` park. Re-introducing first-wins correlation (or an "ignore extras" pattern) is exactly the J-A2-06 mixed-RFQ-data hazard PR-5 closes — the per-counterparty quote could be auto-created against the wrong RFQ when a forwarded thread mixes replies. Repeated SAME id (counterparty quoting their own outbound) collapses to one distinct id and proceeds to single-RFQ correlation; that case is NOT a forwarded thread.
