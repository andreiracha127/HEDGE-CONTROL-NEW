...
def upgrade() -> None:
    ...
    # Partial UNIQUE on (idempotency_key, requested_by) — postgres + sqlite
    # 3.8+ both support. Scoping to requested_by prevents a concurrent submit
    # from a DIFFERENT actor with the same key from being silently deduped
    # against the first actor's row (the consume endpoint then 403s on
    # actor-level authorization, leaving the second actor's mutation
    # unrecoverable). Per-actor scoping makes the idempotency-key invariant
    # actor-local, which matches the real-world semantics (clients carry
    # their own request-deduplication keys, not a shared pool).
    op.create_index(
...
- `test_grant_concurrent_double_grant` — two concurrent threads attempt `grant_request` against the same `pending` row with two DIFFERENT approver `actor_sub` values (both authorized; both != `requested_by`). Only ONE thread's transition commits; the second observes the row as `approved` after the first commit and raises HTTP 409 Conflict from the status check. The `with_for_update()` lock serializes the two attempts identically to `consume_request`. Assertion: exactly ONE `workflow_approval_granted` audit event row, `approved_by` set to whichever thread committed first, no mismatched `approver_ip`/`approver_session_id` overwrites.
- `test_reject_concurrent_double_reject` — two concurrent threads attempt `reject_request` against the same `pending` row with DIFFERENT `(rejection_reason_code, rejection_reason_text)` pairs. Only ONE thread's transition commits; the second observes the row as `rejected` after the first commit and 409s. Assertion: exactly ONE `workflow_approval_rejected` audit event row, `rejection_reason_*` columns reflect whichever thread won the race (no partial overwrite where code is from thread A and text is from thread B). Closes the §4.1 grant/reject concurrency-guard binding the same way `test_consume_concurrent_double_consume` closes consume.
...
3. **Partial UNIQUE on `(idempotency_key, requested_by)` exists.** Postgres: `SELECT indexdef FROM pg_indexes WHERE indexname = 'ux_workflow_approval_requests_idempotency_key'` returns a definition referencing BOTH `idempotency_key` AND `requested_by` columns, with the partial predicate `WHERE (idempotency_key IS NOT NULL)`. SQLite: `SELECT sql FROM sqlite_master WHERE name = 'ux_workflow_approval_requests_idempotency_key'` returns a CREATE INDEX on the same `(idempotency_key, requested_by)` pair. (Enforces amendment lines 1038-1050 + Greptile iter 11 actor-scoped idempotency fix.)
