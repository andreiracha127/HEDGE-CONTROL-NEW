...
- **Reproducibility**: model identifier MUST be a stable identifier documented by Anthropic — either a versioned alias (`claude-sonnet-4-6`, `claude-opus-4-7`, etc., which Anthropic pins to a specific generation) OR a dated form (`claude-sonnet-4-5-20250929`-shape, which pins an intra-version revision). Floating aliases (`claude-sonnet-latest`, `claude-3-5-sonnet-latest`) are forbidden — they re-bind silently. Dated forms are preferred when intra-version revision precision matters; versioned aliases are acceptable when the institutional contract is "this model generation, current revision". Rule-sheet content is versioned in git so any historical review run can be reproduced from `(git SHA, model ID, dispatch SHA)`.
