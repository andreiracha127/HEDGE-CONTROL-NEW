...
- `test_risk_flags_step_idempotent_on_partial_resume` — fixture seeds (a) a counterparty above the unhedged guardrail, (b) a counterparty with KYC regression + active deal, (c) a stale workflow approval. The failure point MUST be INSIDE `_step_risk_flags` itself (NOT inside a later step like `_step_summary` — if `risk_flags` reaches `completed`, the service's resume logic skips it via `if step.status == completed: continue` at finance_pipeline_service.py:76-77, and the anti-join guards are never re-exercised). Concretely: patch `_query_kyc_regressions_with_active_deals` to raise `RuntimeError("simulated DB outage")` on the FIRST call but succeed on subsequent calls; routines 1 and 2 will have flushed their flag rows (1 + 1 = 2 rows committed by the service-level `db.flush()` chain before the failure), then routine 3 raises and the service-level handler at lines 94-101 transitions the `risk_flags` step to `failed` and the run to `partial`. The first invocation returns with `run.status == "partial"` and 2 rows committed in `finance_pipeline_risk_flags`. Now resume by calling `run_daily_pipeline` again with the same `run_date` (the patch on `_query_kyc_regressions_with_active_deals` is lifted so the second call succeeds). The `risk_flags` step re-runs (its status was `failed`, NOT `completed`); routines 1 and 2 anti-join their already-emitted rows (write 0 new); routine 3 writes 1; routine 4 writes 1. Assert the final `finance_pipeline_risk_flags` row count for the run is exactly 4 (NOT 6 — no duplicates of the routine-1-and-2 rows). Assert NO `IntegrityError` was raised at any point (the §5.3 UNIQUE constraint was never tripped because the anti-joins worked). Assert the run lands at `completed` on the second pass. Without the anti-joins in routines 1/2/3/4, the second pass would observe a permanent `IntegrityError` failure loop.
