...
|---|---|---|---|
| 1 | `:692-702` Replay-window check missing on non-exempt live single-event POST paths | Both current POST paths are bulk-exempt (page-scrape with stable-key idempotency) | Add `check_replay_window` helper in `market_data_governance.py` as forward-looking scaffold; NEITHER current POST path invokes it (both bulk-exempt). Code comments document the structural classification per binding's Backfill exemption + Bulk exemption clauses. |
| 2 | `:704-709` Sequence number tracking missing for live single-event ingest paths | No live single-event ingest path exists | Add `check_sequence_monotonicity` helper + `market_data_sequence_tracker` table via alembic 045. NEITHER current POST path invokes the helper. **Plus**: harden the existing bulk-style stable-key idempotency check to enforce §"Bulk idempotency vs replay distinction" (`price_usd` comparison; matching → `market_data_bulk_idempotent_skip`; differing → `market_data_replay_rejected` reason `bulk_content_mismatch`). |
| 3 | `:711-715` No background staleness-check job | None exists | Add `backend/app/tasks/market_data_staleness_task.py` + per-pair `max_gap_hours` config + `MARKET_DATA_STALENESS_CHECK_INTERVAL_MINUTES` cadence. Emit `market_data_stale_feed` warning when gap exceeds threshold. Westmetall+LME_ALU_CASH_SETTLEMENT_DAILY default `max_gap_hours = 36` (allows for 1-day weekend gap). |
...
51. **`test_daily_for_date_matching_existing_emits_idempotent_skip`** — pre-seed DB with one row at `Decimal("2567.50")`; mock fetch to return the same price for that date; ingest → no new row, structured event `market_data_bulk_idempotent_skip` emitted with correct fields, returns `(None, 0, 1, evidence)`.
...
63. **`test_staleness_check_emits_alert_when_gap_exceeds`** — seed DB with a row at `now - 48h`, default threshold 36h; run task; event emitted with `gap_hours ≈ 48`, `max_gap_hours == 36`.
